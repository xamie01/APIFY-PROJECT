"""
O-SATE Streamlit Web Frontend - Demo Version
A modern, beautiful web interface for the O-SATE AI safety testing platform
This is a demo version that showcases the UI without requiring all dependencies
"""
...
def render_home():
    ...
    with col1:
        ...
        <div class="feature-card">
            ...
            <p style="color: #6b7280;">
                Test AI models from multiple providers. Select from various free models 
...
def render_ai_testing():
    ...
    if submit_button and prompt and selected_model:
        with st.spinner("🔄 Querying AI model..."):
            ...
            # Demo response
            response = f"""This is a demo response from {selected_model}.

In a production environment, this would be the actual AI model's response to your prompt:
"{prompt}"

The model would process your input using the following parameters:
- Temperature: {temperature}
- Max Tokens: {max_tokens}

To enable full functionality, please configure your API keys and ensure all dependencies are installed."""
            ...
            with col1:
                st.metric("Response Time", "1.23s")
            
            with col2:
                st.metric("Total Requests", len(st.session_state.ai_wrappers) + 1)
            
            with col3:
                st.metric("Avg Response Time", "1.15s")
        ...
        if st.button("🚀 Create Sandbox", use_container_width=True):
            if not sandbox_name:
                ...
                with st.spinner("Creating sandbox..."):
                    time.sleep(1)
                    st.success(f"✅ Sandbox '{sandbox_name}' created successfully! (Demo)")
                    st.info("Container ID: demo-container-id")
        ...
        if st.button("▶️ Execute", use_container_width=True):
            if not sandbox_name_exec or not command:
                ...
                with st.spinner("Executing command..."):
                    ...
                    st.markdown("### 📤 Output")
                    demo_output = f"Demo output for command: {command}\n\nExecution completed successfully."
                    st.code(demo_output, language="bash")
                    st.success("✅ Command executed successfully! (Demo)")
        ...
        if st.button("📊 Get Statistics", use_container_width=True):
            if not sandbox_name_stats:
                ...
                with st.spinner("Fetching statistics..."):
                    ...
                    with col1:
                        st.metric("CPU Usage", "15.3%")
                    with col2:
                        st.metric("Memory Usage", "128.5 MB")
                    with col3:
    ...
    if st.button("🚀 Run Safety Tests", use_container_width=True, type="primary"):
        if not test_model or not prompts:
            ...
            for i, prompt in enumerate(prompts):
                ...
                results.append({
                    'prompt': prompt,
                    'response': f"Demo response to: {prompt}\n\nThis would be the actual AI model's response in production mode.",
                    'success': True,
